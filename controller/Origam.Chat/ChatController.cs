...
public class ChatController : ControllerBase
{
    ...
    private readonly Guid OrigamChatMessageDataStructureId = 
        new Guid("f9ec17ce-13fc-420c-88b0-3a793fae4001");
    private readonly Guid OrigamChatMessageDataAfterIdIncludingMethodId 
        = new Guid("61544d12-c4ec-4b05-83c7-427b8cc8bb32");
    private readonly Guid OrigamChatMessageDataBeforeIncludingMethodId 
        = new Guid("338cca32-f6d2-454f-b4b0-e33a39a1a3cb");
    private readonly Guid OrigamChatMessageDataGetByIdMethodId 
        = new Guid("88542750-75e0-4d48-b271-793126cf4847");
    private readonly Guid OrigamChatMessageDataGetByRoomIdMethodId 
        = new Guid("42076303-19b4-4afa-9836-d38d87036c29");
    private readonly Guid OrigamChatMessageDataOrderByCreatedDateSortSetId 
        = new Guid("72c14783-0667-4827-b4ee-137e17fe0e3e");

    private readonly Guid OrigamChatRoomDatastructureId 
        = new Guid("ae1cd694-9354-49bf-a68e-93b1c241afd2");
    private readonly Guid OrigamChatRoomGetById 
        = new Guid("a2d56dbe-6807-48ef-9765-7fd8cdaa09c7");

    private readonly Guid OrigamChatRoomBusinessPartnerId 
        = new Guid("fab83217-6cb4-4693-b10e-6ded25a23abf");
    private readonly Guid OrigamChatRoomBusinessPartnerGetParticipantsId 
        = new Guid("f7d1096f-4dfc-4b66-bef5-f54c7a1d9ee1");
    private readonly Guid OrigamChatRoomBusinessPartner_GetBusinessPartnerId 
    ...
    private readonly Guid DefaultBusinessPartner 
        = new Guid("ab4eb78c-6dcf-46c7-a316-e856f835fbfa");
    private readonly Guid LookupBusinessPartnerGetByIdWithoutMe 
        = new Guid("34dcbb06-b353-42ff-9f33-f03478eb7ece");
    private readonly Guid LookupBusinessPartnerGetAllUsers 
        = new Guid("73bd2418-fc6c-4cf4-b44b-ad2084e25af9");

    private readonly Guid OnlineUsers 
        = new Guid("aa4c9df9-d6da-408e-a095-fd377ffcc319");
    private readonly Guid OrigamChatMessageBusinessPartnerDataStructureId 
        = new Guid("763e029f-b306-40bd-98f9-36f89297cfbf");
        ...
        [FromQuery]Guid beforeIdIncluding)
    {
        return RunWithErrorHandler(() => 
            Ok(GetMessages(requestChatRoomId, limit, 
                afterIdIncluding, beforeIdIncluding)));
    }
    [HttpGet("chatrooms/{requestChatRoomId:guid}/info")]
    public IActionResult GetChatRoomInfoRequest(Guid requestChatRoomId)
    {
        return RunWithErrorHandler(() => 
            Ok(GetChatRoomInfo(requestChatRoomId)));
    }
    [HttpGet("chatrooms/{requestChatRoomId:guid}/participants")]
    public IActionResult GetChatRoomParticipantsRequest(
        Guid requestChatRoomId)
    {
        return RunWithErrorHandler(() => 
            Ok(GetChatRoomParticipants(requestChatRoomId)));
    }
    ...
    {
        return RunWithErrorHandler(() => 
            GetUsersToInviteToNewRoom(limit, offset, searchPhrase));
    }
    ...
    {
        return RunWithErrorHandler(() => 
            GetPollData(requestChatRoomId, limit, afterIdIncluding, 
                beforeIdIncluding));
    }
    [HttpGet()]
    ...
    public IActionResult CreateRoomRequest(
        [FromBody]NewChatRoom newRoomJson)
    {
    ...
    {
        return RunWithErrorHandler(() => 
            PostMessages(requestChatRoomId, chatMessages));
    }
    ...
    {
        return RunWithErrorHandler(() => 
            PostRoomAbandon(requestChatRoomId, outviteUser));
    }
    ...
    {
        return RunWithErrorHandler(() => 
            PostRoomChangeTopic(requestChatRoomId, topic));
    }
    private IActionResult PostRoomChangeTopic(
        Guid requestChatRoomId, 
        Info topic)
    {
    ...
    private Guid CreateRoomDatabase(NewChatRoom newChatRoom)
    {
        ...
        var dataStructure = (DataStructure)persistenceService.SchemaProvider
        ...
        List<InviteUser> inviteUsers)
    {
        ...
        var persistenceService 
            = ServiceManager.Services.GetService<IPersistenceService>();
        var dataStructure = (DataStructure)persistenceService
    ...
    private IActionResult PostRoomAbandon(
        Guid requestChatRoomId, OutviteUser outviteUser)
    {
        var datasetUsersForInvite 
            = GetActiveUserChatRoom(requestChatRoomId, outviteUser);
    ...
    private DataSet GetActiveUserChatRoom(
        Guid requestChatRoomId, OutviteUser outviteUser)
    {
    ...
    private IActionResult PostInviteUser(
        Guid requestChatRoomId, Guid userId)
    {
        var users = new List<InviteUser> {new InviteUser(userId)};
        AddUsersIntoChatRoom(requestChatRoomId, users);
        return Ok();
    }
    private IActionResult GetUsersToInviteToNewRoom(
        int limit, int offset, string searchPhrase)
    {
        ...
        {
            parameters.Add(
                new QueryParameter("BusinessPartner_parSearchText", 
                    searchPhrase));
        ...
        var datasetUsersForInvite = LoadData(LookupBusinessPartner, 
            methodId, Guid.Empty, Guid.Empty, 
            null, parameters);
        ...
        bool usersNotExistsInRoom)
    {
        ...
        var datasetUsersForInvite = LoadData(
            LookupBusinessPartner, methodId, Guid.Empty, 
            Guid.Empty, null, parameters);
        return Ok(OrigamChatBusinessPartner.CreateJson(
            datasetUsersForInvite, participants, usersNotExistsInRoom));
    }
    ...
    private object GetLocalUser()
    {
        ...
        return new OrigamChatBusinessPartner(
            profile.Id, profile.FullName, profile.Id.ToString());
    }
    private List<OrigamChatParticipant> GetChatRoomParticipants(
        Guid requestChatRoomId)
    {
        var parameters = new QueryParameterCollection 
        {
        ...
        var datasetParticipants = LoadData(
            ...
            Guid.Empty, Guid.Empty, null, 
            parameters);
        var onlineUsers = LoadData(OnlineUsers,Guid.Empty,
            Guid.Empty, Guid.Empty, null, 
            null);
        return OrigamChatParticipant.CreateJson(
            datasetParticipants, onlineUsers);
    }
    ...
    private DataSet GetChatRoom(Guid requestChatRoomId)
    {
        ...
        var datasetRoom = LoadData(OrigamChatRoomDatastructureId, 
            OrigamChatRoomGetById, Guid.Empty, 
            Guid.Empty, null, parameters);
        return datasetRoom;
    }
    private IActionResult PostMessages(
        Guid requestChatRoomId, OrigamChatMessage chatMessages)
    {
        var profile = SecurityManager.CurrentUserProfile();
        var datasetGenerator = new DatasetGenerator(true);
        var persistenceService 
            = ServiceManager.Services.GetService<IPersistenceService>();
        var messageDataStructure = (DataStructure)persistenceService
            .SchemaProvider.RetrieveInstance(typeof(AbstractSchemaItem), 
        ...
        foreach (var messageMention in chatMessages.Mentions)
        {
            var messageBusinessPartnerRow 
                = messageBusinessPartnerDataSet.Tables[
                    "OrigamChatMessageBusinessPartner"].NewRow();
        ...
        Guid beforeIdIncluding)
    {
        ...
        pollData.Add("messages", GetMessages(
            requestChatRoomId, limit, afterIdIncluding, beforeIdIncluding));
    ...
    private List<OrigamChatMessage> GetMessages(
        Guid requestChatRoomId, int limit, 
        Guid afterIdIncluding, Guid beforeIdIncluding)
    {
        ...
        if(includingId != Guid.Empty)
        {
            ...
            parametersMessages.Add(
                new QueryParameter("OrigamChatMessage_parCreatedDateTime", 
                getRecordCreated.Tables[0].Rows[0]["RecordCreated"]));
        }
        parametersMessages.Add(
            new QueryParameter("OrigamChatMessage_parOrigamChatRoomId", 
                requestChatRoomId));
        if(limit > 0)
        {
            parametersMessages.Add(
                new QueryParameter("OrigamChatMessage__pageNumber", 0));
            parametersMessages.Add(
                new QueryParameter("OrigamChatMessage__pageSize", limit));
        }
        var messagesDataSet = LoadData(
            OrigamChatMessageDataStructureId, methodId,
            Guid.Empty, OrigamChatMessageDataOrderByCreatedDateSortSetId, 
            null, parametersMessages);
        return OrigamChatMessage.CreateJson(messagesDataSet, allUsers);
    }
    private DataSet LoadData(
        Guid dataStructureId, Guid methodId, Guid defaultSetId, 
